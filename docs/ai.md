...
url = 'http://example.com/api/ai/v1/text/sql'
headers = {
    'Content-Type': 'application/json'
}
data = {
...
```bash
curl -X POST http://example.com/api/ai/v1/text/sql \
-H "Content-Type: application/json" \
-d '{
  "input": "What are my total sales by region?",
  "dataSourceSchemas": [
...
response = requests.post('http://your-api-domain.com/api/ai/v1/text/generation', headers=headers, json=data)
print(response.json())
```
...
curl -X POST 'http://your-api-domain.com/api/ai/v1/text/generation' \
-H 'Content-Type: application/json' \
...
| `modelConfiguration` | object            | No       | Additional model-specific configuration parameter key-value pairs. e.g. temperature, max_tokens, etc.                                             |
| `system`             | string            | No       | The system message to use for the Text-to-SQL task. If not provided, the default system will be used. If the model does not include built-in      |
| `temperature`        | number (double)   | No       | Controls randomness in the model's output. Lower values make output more deterministic.                                                           |
| `maxTokens`          | integer (int32)   | No       | The maximum number of tokens to generate in the response.                                                                                        |
...
```bash
curl -X POST /api/ai/v1/text/beastmode \
-H "Content-Type: application/json" \
-d '{
  "input": "Count distinct products",
  "dataSourceSchema": {
...
response = requests.post(
    url='https://yourapiurl.com/api/ai/v1/messages/tools',
    headers={'Content-Type': 'application/json'},
    json={
        "input": [
            {
...
```bash
curl -X POST https://yourapiurl.com/api/ai/v1/messages/tools \
  -H 'Content-Type: application/json' \
  -d '{
    "input": [
      {
...
fetch('/api/ai/v1/image/text', {
  ...
  body: JSON.stringify({
    ...
    sessionId: "4df12f28-bdb1-491b-b820-b46ab1cd5969",
